...
// Link with Intel MKL library: With MSFT VS and Intel Composer integration: Select build components in the Project context menue.
...
void multiply5(int msize, int tidx, int numt, TYPE a[][NUM], TYPE b[][NUM], TYPE c[][NUM], TYPE t[][NUM])
{
	...
	if(tidx == 0) // do in a sigle thread if linked with multithreaded version of MKL library
	{
		double alpha = 1.0, beta = 0.;
		cblas_dgemm(CblasRowMajor,CblasNoTrans,CblasNoTrans,NUM,NUM,NUM,alpha,b,NUM,a,NUM,beta,c,NUM);
	}
}
#endif //USE_MKL