...
multiply4 - Optimized implementation with matrix transposition and loop un-rolling
multiply5 – Most optimal version with using Intel MKL (link the MKL library to the progect and add the USE_MKL macro to Compiler preprocessor)
...
For the MKL-based kernel consider either MKL's multithreaded or singlethreded implementation. Make the kernel multithreaded by youself in the latter case. 
...
By default # of thereads executed is eqial to number of CPU cores available in the system (defined in run-time). You may want to limit max number of created threads by modifying the MAXTHREADS.
...
Windows:
Use the buildmatrix.bat butch file from the windows_mic subdirectory (see the comments inside the buildmatrix.bat). 
